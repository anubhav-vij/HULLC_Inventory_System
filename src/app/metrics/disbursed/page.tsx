...
export default function MetricsDisbursedPage() {
  const router = useRouter();
  const { toast } = useToast();
  const [user, setUser] = useState<User | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [data, setData] = useState<DisbursedRow[]>([]);
  ...
  const fetchData = useCallback(async (from: string, to: string) => {
    ...
    try {
      const res = await fetch(
        `/api/metrics/disbursed?from=${from}&to=${to}`,
      ...
      setData([]);
      toast({ title: "Load Error", description: "Could not load disbursed inventory data.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  }, [user]);
  ...
  const sortedData = useMemo(() => {
    return [...data].sort((a, b) => {
      const da = new Date(a.dispensed_date).getTime();
      const db = new Date(b.dispensed_date).getTime();
      return sortAsc ? da - db : db - da;
    });
  }, [data, sortAsc]);

  const totalTransactions = data.length;
  const totalQuantity = data.reduce((sum, r) => sum + r.quantity_dispensed, 0);
  ...
  const handleExport = () => {
    ...
    const rows = sortedData.map((r) => ({
      ...
      "Qty Dispensed": r.quantity_dispensed,
      "Dispensed By": r.dispensed_by,
      Project: r.project_name,
      "Functional Group": r.functional_group,
      Date: format(new Date(r.dispensed_date), "yyyy-MM-dd"),
    }));
    ...
    XLSX.writeFile(wb, `inventory-disbursed-${appliedFrom}-to-${appliedTo}.xlsx`);
  };
  ...
  return (
    <div className="min-h-screen w-full" style={{ backgroundColor: "#eef2f7" }}>
      <Sidebar
        ...
        onNavigate={(view) => {
          if (view === "metrics-disbursed") return;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: "#64748b" }}>
                Items dispensed from inventory within a date range
              </p>
            </div>
          </div>
                      ...
                      <TableBody>
                        {sortedData.map((r, i) => (
                          <TableRow key={i}>
                            ...
                            <TableCell>{r.dispensed_by || "—"}</TableCell>
                            <TableCell>{r.project_name || "—"}</TableCell>
                            <TableCell>{r.functional_group || "—"}</TableCell>
                            <TableCell>{format(new Date(r.dispensed_date), "yyyy-MM-dd")}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
